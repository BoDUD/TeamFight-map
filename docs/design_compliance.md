...
The current repository had no existing game integration layer, so this PR adds a data-first implementation and validation harness. Runtime hooks, asset packing, and in-game path probes should build on this validated layout in the next implementation stage.
