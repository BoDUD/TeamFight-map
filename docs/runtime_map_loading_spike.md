...
This proves the background visual asset can be overridden through `mod.override_info`, and that the loader registers and reads a byte-equivalent `map_setting` override when staged with the `.map_setting` file extension. It does not prove collision, lane pathing, spawn points, brush gameplay regions, objective placement, or `map_setting` mutation.
...
7. Next: decode `map_setting`, re-encode without field edits, and prove the output is byte-identical to the original.
...
The next gate after Q2a loader takeover is:
