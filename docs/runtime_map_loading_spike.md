...
11. Next: prove an independent `map_setting` node/world anchor through SDK/runtime logging, debug drawing, or decoded original entity/path data.
12. Later only if node/world anchoring is resolved: try one tiny reversible mutation in a separate branch or PR with A/B/A runtime proof.
...
The next evidence item should be an independent node/world anchor, not a mutated `map_setting`.
