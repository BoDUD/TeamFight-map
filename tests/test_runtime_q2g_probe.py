...
from tools import install_runtime_q2g_probe, install_runtime_spike_mod


class RuntimeQ2gProbeTests(unittest.TestCase):
    ...
    def write_q2g_pair(self, root: Path) -> tuple[Path, Path]:
        size = max(install_runtime_q2g_probe.EXPECTED_MUTATION_OFFSETS) + 1
        original = bytearray(size)
        for offset in install_runtime_q2g_probe.EXPECTED_MUTATION_OFFSETS:
            original[offset] = 1
        mutated = bytearray(original)
        for offset in install_runtime_q2g_probe.EXPECTED_MUTATION_OFFSETS:
            mutated[offset] = 0
        original_path = self.write_source(root, "original.map_setting", bytes(original))
        mutated_path = self.write_source(root, "map_setting.q2g.mutated.map_setting", bytes(mutated))
        return original_path, mutated_path

    def write_q2g_manifest(self, path: Path, original: Path, source: Path, **overrides: object) -> None:
        manifest = {
            ...
            "expected_changed_offsets": [66605, 932331],
            "changed_cells": install_runtime_q2g_probe.EXPECTED_MUTATION_CELLS,
    ...
    def test_stage_a1_original_map_setting(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            old_baseline = install_runtime_q2g_probe.BASELINE_SHA256
            install_runtime_q2g_probe.BASELINE_SHA256 = expected_sha
            try:
                manifest = install_runtime_q2g_probe.stage_runtime_q2g_probe(
                    game_root=game_root,
                    mode="A1",
                ...
                install_runtime_q2g_probe.BASELINE_SHA256 = old_baseline
            ...
            self.assertEqual("q2g_second_candidate_loader_probe_stage", manifest["probe"])
            self.assertEqual("59-837", manifest["candidate"])
            ...
            self.assertTrue((root / "evidence" / "q2g_a1_stage_manifest.json").is_file())
    ...
    def test_stage_b_requires_manifest_and_validates_q2g_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            game_root = self.make_game_root(temp_dir)
            original_path, source = self.write_q2g_pair(root)
            mutated = source.read_bytes()
            manifest_path = root / "evidence" / "q2g_mutation_manifest.json"
            self.write_q2g_manifest(manifest_path, original_path, source)
            old_baseline = install_runtime_q2g_probe.BASELINE_SHA256
            install_runtime_q2g_probe.BASELINE_SHA256 = install_runtime_spike_mod.sha256_file(original_path)
            try:
                manifest = install_runtime_q2g_probe.stage_runtime_q2g_probe(
                    game_root=game_root,
                    mode="B",
            ...
            self.assertEqual("q2g_second_candidate_two_byte_mutation", manifest["stage"])
            self.assertEqual(install_runtime_spike_mod.sha256_file(source), manifest["mutation_manifest_output_sha256"])
            self.assertEqual([66605, 932331], manifest["approved_changed_offsets"])
            self.assertEqual(install_runtime_q2g_probe.EXPECTED_MUTATION_CELLS, manifest["approved_changed_cells"])
            self.assertTrue((root / "evidence" / "q2g_b_stage_manifest.json").is_file())

    def test_stage_b_rejects_wrong_candidate_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            game_root = self.make_game_root(temp_dir)
            original, source = self.write_q2g_pair(root)
            manifest_path = root / "evidence" / "q2g_mutation_manifest.json"
            self.write_q2g_manifest(manifest_path, original, source, candidate="369-370")
            old_baseline = install_runtime_q2g_probe.BASELINE_SHA256
            install_runtime_q2g_probe.BASELINE_SHA256 = install_runtime_spike_mod.sha256_file(original)
            try:
                with self.assertRaises(SystemExit) as raised:
                    install_runtime_q2g_probe.stage_runtime_q2g_probe(
                        game_root=game_root,
                        mode="B",
                        map_setting_source=source,
                        evidence_dir=root / "evidence",
    ...
    def test_stage_b_rejects_bad_offsets_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            game_root = self.make_game_root(temp_dir)
            original, source = self.write_q2g_pair(root)
            manifest_path = root / "evidence" / "q2g_mutation_manifest.json"
            self.write_q2g_manifest(manifest_path, original, source, changed_offsets=[1, 2])
            old_baseline = install_runtime_q2g_probe.BASELINE_SHA256
            install_runtime_q2g_probe.BASELINE_SHA256 = install_runtime_spike_mod.sha256_file(original)
            try:
                with self.assertRaises(SystemExit) as raised:
    ...
    def test_stage_b_rejects_actual_diff_outside_approved_offsets(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            manifest_path = root / "evidence" / "q2g_mutation_manifest.json"
            self.write_q2g_manifest(manifest_path, original, source)
            old_baseline = install_runtime_q2g_probe.BASELINE_SHA256
            install_runtime_q2g_probe.BASELINE_SHA256 = install_runtime_spike_mod.sha256_file(original)
            try:
                with self.assertRaises(SystemExit) as raised:
    ...
    def test_non_clean_existing_map_setting_override_fails_before_replacing_target(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertFalse((root / "evidence" / "q2g_b_stage_manifest.json").exists())
