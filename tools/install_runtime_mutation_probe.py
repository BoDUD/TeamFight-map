...
from tools import install_runtime_spike_mod as spike
from tools import map_setting_round_trip as rt


MODE_TO_MANIFEST = {
    "A1": "q2e_a1_stage_manifest.json",
    "B": "q2e_b_stage_manifest.json",
    "A2": "q2e_a2_stage_manifest.json",
}
BASELINE_SHA256 = rt.MAP_SETTING_SHA256
EXPECTED_MUTATION_OFFSETS = [427536, 427573]
...
DEFAULT_EVIDENCE_DIR = Path("D:/tfm2_q2a_evidence/minimal_mutation_probe")
...
def validate_mutation_source(source: Path, mutation_manifest: dict[str, Any]) -> str:
    ...
    if mutation_manifest.get("changed_offsets") != EXPECTED_MUTATION_OFFSETS:
        raise SystemExit("Mutation manifest changed_offsets do not match the approved Q2e offsets.")
    if mutation_manifest.get("expected_changed_offsets") != EXPECTED_MUTATION_OFFSETS:
        raise SystemExit("Mutation manifest expected_changed_offsets do not match the approved Q2e offsets.")
    if mutation_manifest.get("changed_cells") != EXPECTED_MUTATION_CELLS:
        raise SystemExit("Mutation manifest changed_cells do not match the approved Q2e cells.")
    ...
    if len(mutated_data) <= max(EXPECTED_MUTATION_OFFSETS):
        raise SystemExit("Mode B source is too small to contain the approved Q2e offsets.")
    if diff_offsets != EXPECTED_MUTATION_OFFSETS:
        raise SystemExit(f"Mode B actual byte diff {diff_offsets} does not match the approved Q2e offsets.")
    for cell in EXPECTED_MUTATION_CELLS:
        offset = cell["offset"]
        if baseline_data[offset] != cell["old"] or mutated_data[offset] != cell["new"]:
            raise SystemExit("Mode B actual old/new byte values do not match the approved Q2e mutation.")
    return source_sha
...
def load_base_override_for_staging(installed_mod: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if set(overrides) != set(spike.DEFAULT_VISUAL_ASSETS):
        raise SystemExit("Runtime mutation staging requires the default visual-only overrides.")
...
def override_with_map_setting(override_path: Path, overrides: dict[str, Any]) -> Path:
    ...
    if set(overrides) != set(spike.DEFAULT_VISUAL_ASSETS) | {spike.MAP_SETTING_ASSET}:
        raise SystemExit("Runtime mutation staging allows only default visual overrides and map_setting.")
    override_path.write_text(json.dumps(overrides, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
    return override_path


def stage_runtime_mutation_probe(
    game_root: Path,
    mode: str,
    ...
    if set(installed_overrides) != set(spike.DEFAULT_VISUAL_ASSETS) | {spike.MAP_SETTING_ASSET}:
        raise SystemExit("Installed override contains unexpected assets after Q2e staging.")
    ...
    manifest = {
        "probe": "q2e_loader_mutation_probe_stage",
        "mode": mode,
    ...
    print(f"Staged Q2e {mode} map_setting probe in installed mod: {target}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage one Q2e A/B/A map_setting runtime mutation probe.")
    ...
    manifest = stage_runtime_mutation_probe(
        game_root=game_root,
        mode=args.mode,
